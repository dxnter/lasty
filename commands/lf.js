...
module.exports.run = async (bot, message, args) => {
  ...
  case 'help': {
    return message.channel.send(
      new Discord.RichEmbed()
        ...
        .addField('set - Sets Last.FM username.', 'Example: `,lf set iiMittens`')
        .addField('delete - Deletes saved Last.FM username', 'Alternate: `reset`')
        .addField('info - Shows Last.FM account information', 'Example: `,lf info`')
  ...
  case 'set': {
    ...
    if (existingUser) {
      ...
      db
        ...
        .write();
      return message.channel.send(`Last.FM username updated to **${fmUser}**`);
    }
    db
        .get('users')
  ...
  case 'info': {
    ...
    const { totalScrobbles, name, profileURL, country, image, unixRegistration } = await getUserInfo(fmUser);
  ...
  case 'reset': {
    ...
    if (existingUser) {
      db
        ...
        .write();
      return message.channel.send(`**${existingUser.lastFM}** has been deleted from the database`);
    }
    return message.channel.send(
      `<@${
        message.author.id
  ...
  case 'artists': {
    const topArtists = await getUsersTopArtists(fmUser, period, message, args);
  ...
  case 'topalbums': {
    const { artistTopAlbums, formattedArtist, artistURL } = await getArtistTopAlbums(args);
    return message.channel.send(
      new Discord.RichEmbed()
  ...
  default: {
    return message.channel.send(`<@${message.author.id}>, Invalid command, try \`,lf help\``);
  }
  }
};
